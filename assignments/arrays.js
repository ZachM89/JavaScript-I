...
//console.log("Car 33 is a " + inventory[0].id + " *car year goes here* *car make goes here* *car model goes here*");
...
for(let a=0; a<inventory.length;a++){
    carModels.push(inventory[a].car_model);
}
let carModelsSorted = carModels;
...
let carYears = [];
for(let a=0; a<inventory.length;a++){
    carYears.push(inventory[a].car_year);
}
...
let BMWAndAudi = [];
for(let a=0; a<inventory.length;a++){
    if(inventory[a].car_make == "BMW" || inventory[a].car_make == "Audi"){
        BMWAndAudi.push(inventory[a]);
    }
}
